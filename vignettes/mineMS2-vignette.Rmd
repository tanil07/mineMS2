...
```{r graph_mining}
m2l <- mineClosedSubgraphs(m2l,num=5,sizeMin = 2, precursor = FALSE)
```
#IDs
...
plotOccurences(m2l,"P10",byPageMax=6)

```
#Scoring the patterns
Two scores are calculated by *mineMS2* a score for each individual loss and a score for each pattern :
```{r scoring_edge}
m2l <- scoresLosses(m2l)
