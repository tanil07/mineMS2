...
```{r pattern_computing}
m2l <- ms2Lib(path_mgf,intThreshold = 4000)
...
MineMS2 implement an matching between components and subgraphs occurences using the [F1-score](https://en.wikipedia.org/wiki/F1_score). For each component the pattern maximizing the F1-score mays be extracted using the _findPatternsExplainingComponents_ function. We consider that a good match is found if there is an F-score superior to 0.8 between the two components.
```{r patterns_explaining_components}
patterns <- findPatternsExplainingComponents(m2l,components,metric=c("recall"),threshold=0.8)
```
