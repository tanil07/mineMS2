...
## Input 
The _mineMS2_ package takes as input the following format `r recognisedFormat()` or may also may also take as input a list _Spectrum2_ object from the _MSnbase_ package. 
...
This file should contain as much rows as spectra. 
...
* _maxFrags_: only the _maxFrags_ most intense peaks will be considered to build the graph (shoul be at most 20, to be done in reasonable time).
...
* _heteroAtoms_: Shall hetero atoms (*P*,*S*,*Cl*) be used when generating formula for mass differences.

* _limMzFormula_: the range of mass difference values for which formulas will be searched. Higher values will be considered as too high to find a set of formula without ambiguity. They are however kept as mass differences.
...
The step of discretization also constructs the directed acyclic graphs (DAGs) corresponding to the spectra, with vertices as peaks and directed edges as discretized mass differences. 
...
### Spectra
Spectra may be queried using id starting by "S". It is possible to find a specific spectra by using the _findMz_ function, with the "S" parameter.
